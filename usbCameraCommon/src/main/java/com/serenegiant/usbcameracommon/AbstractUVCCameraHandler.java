...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	public void startRecording(String path) {
		checkReleased();
		sendMessage(obtainMessage(MSG_CAPTURE_START, new OpenVideoTag(path,true)));
	 ...
	 * @param isOpenAudio
     */
	public void startRecording(boolean isOpenAudio,String path) {
		checkReleased();
		sendMessage(obtainMessage(MSG_CAPTURE_START, new OpenVideoTag(path,isOpenAudio)));
		//sendEmptyMessage(MSG_CAPTURE_START);
	}
	...
	public void handleMessage(final Message msg) {
		...
		case MSG_CAPTURE_START:
			thread.handleStartRecording((OpenVideoTag) msg.obj);
			break;
	...
	static final class CameraThread extends Thread {
		...
		public void handleStartRecording(OpenVideoTag tag) {
			if (DEBUG) Log.v(TAG_THREAD, "handleStartRecording:");
			try {
				if ((mUVCCamera == null) || (mMuxer != null)) return;

				MediaMuxerWrapper muxer=null ;
				if (TextUtils.isEmpty(tag.getFilePath()))
					muxer = new MediaMuxerWrapper(".mp4");    // if you record audio only, ".m4a" is also OK.
				else
					muxer=new MediaMuxerWrapper(tag.getFilePath(),"bitch");
